...
- (void)updateTimeLabel
{
    ...
    // 3.显示最后更新日期
    _lastUpdateTimeLabel.text = [NSString stringWithFormat:@"%@：%@",NSLocalizedString(@"MJRefreshLastTimeNotie",@"MJRefreshLastTimeNotie"),time];
}
...
- (void)setState:(MJRefreshState)state
{
	...
	switch (state) {
        ...
        {
            // 设置文字
            _statusLabel.text = NSLocalizedString(@"MJRefreshHeaderReleaseToRefresh", @"MJRefreshHeaderReleaseToRefresh");
            // 执行动画
        ...
        {
            // 设置文字
			_statusLabel.text = NSLocalizedString(@"MJRefreshHeaderPullToRefresh", @"MJRefreshHeaderPullToRefresh");
            // 执行动画
            [UIView animateWithDuration:MJRefreshAnimationDuration animations:^{
                _arrowImage.transform = CGAffineTransformIdentity;
                UIEdgeInsets inset = _scrollView.contentInset;
                DLog(@"_scrollViewInitInset.top下拉可以刷新 = %f",_scrollViewInitInset.top);
                /*yxm modify before 2014年03月11日09:51:31*/
//                inset.top = _scrollViewInitInset.top;
                DLog(@"_scrollViewInitInset.top = %lf ",_scrollViewInitInset.top);
                /*yxm modify after 2014年03月11日09:52:31*/
                //inset.top = _scrollViewInitInset.top-MJRefreshViewHeight;
                /*yxm modify after 2014年03月11日09:53:31*/
                inset.top = 0;
                _scrollView.contentInset = inset;
            }];
        ...
        {
            // 设置文字
            _statusLabel.text = NSLocalizedString(@"MJRefreshHeaderRefreshing", @"MJRefreshHeaderRefreshing");
            // 执行动画
            [UIView animateWithDuration:MJRefreshAnimationDuration animations:^{
                @try {
                    _arrowImage.transform = CGAffineTransformIdentity;
                    ...
                    _scrollView.contentInset = inset;
                    // 2.设置滚动位置
                    DLog(@"_scrollViewInitInset.top正在刷新中 = %f",_scrollViewInitInset.top);
...
- (CGFloat)validY
{
    DLog(@"_scrollViewInitInset.top = %f",_scrollViewInitInset.top);
