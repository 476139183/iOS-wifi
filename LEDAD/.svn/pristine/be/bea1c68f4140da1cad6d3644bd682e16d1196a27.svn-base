...
+ (CGContextRef)CreateARGBBitmapContextWithSize:(CGSize)size {
    ...
    if (bitmapData == NULL) {
		DLog(@"Malloc failed which is too bad.  I was hoping to use this memory.");
...
- (void)drawInRect:(CGRect)r {
	...
	///DLog(@"Height: %d", CGBitmapContextGetHeight(context));
