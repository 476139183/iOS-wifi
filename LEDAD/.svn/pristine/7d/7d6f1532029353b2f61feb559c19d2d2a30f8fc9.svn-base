        ...
        id <SDWebImageOperation> operation = [SDWebImageManager.sharedManager downloadWithURL:url options:options progress:progressBlock completed:^(UIImage *image, NSError *error, SDImageCacheType cacheType, BOOL finished) {
            ...
            dispatch_main_sync_safe(^{
                ...
                if (image) {
                    ...
                    NSString *fromWhere = [ud objectForKey:@"From"];
                    if ([fromWhere isEqualToString:@"HomepageTableViewCell"]) {
                        wself.image = [MyTool scale:image toSize:CGSizeMake(60, 60)];
                    }else if ([fromWhere isEqualToString:@"NewsTableViewCell"]) {
                        wself.image = [MyTool scale:image toSize:CGSizeMake(120, 120)];
                    }else if ([fromWhere isEqualToString:@"AD"]) {
                        wself.image = image;
                    }
